...
class CampaignBase(CampaignUI, Map, AutoSearchCombat):
    ...
    def map_get_info(self):
        """
        Turn the internal clear-all switch into a user facing option.

        Alas derives MAP_CLEAR_ALL_THIS_TIME from the star requirement and the
        stop condition alone, see FastForwardHandler.map_get_info(), so on a
        map that already has its third star the clear-all branch of
        battle_function() is never reached, whatever the user picks. Forcing it
        on here is all this option has to do, every step afterwards is stock
        Alas: clear enemies, sirens and fortresses until none of them is left,
        only then engage the boss, and pick up mysteries along the way.

        Auto search is dropped as a consequence, handled by
        handle_fast_forward() right after this call, because auto search picks
        its own targets and can not be steered.

        The value is logged a second time on purpose. The first line comes from
        map_show_info() inside super() and still shows the derived value, this
        one shows what the run actually uses.

        Pages:
            in: Map preparation, before the sortie starts
        """
        super().map_get_info()

        if self.config.Campaign_ClearAllMap:
            self.config.MAP_CLEAR_ALL_THIS_TIME = True
            logger.attr('MAP_CLEAR_ALL_THIS_TIME (clear all option)', True)
